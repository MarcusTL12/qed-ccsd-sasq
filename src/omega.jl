...
function make_omega(proj, Ω_name, symmetry=[])
    println("\n\nH: $Ω_name += \n", omega(proj, HFe, 2, symmetry))
    println("\n\nω: $Ω_name += \n", omega(proj, H_ω, 2, symmetry))
    println("\n\ned:$Ω_name += \n", omega(proj, H_Eb, 3, symmetry))
end
...
function omega_s2_2()
    make_omega(b^2 * deex_braop(1, 2, 3, 4), "Ω2_aibj", [(1, 2), (3, 4)])
end
