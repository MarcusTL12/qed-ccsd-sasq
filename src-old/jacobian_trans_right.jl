...
function jacobian_right_transformation(left_op, H, symmetry, symbol, name, H_name, indices)
    t1 = simplify_heavy(∑(jacobian_element(left_op, ex_ketop(5, 6), H) * real_tensor("ct", 5, 6), 5:6))
    @time begin
    ...
    t2 = simplify_heavy(1 // 2 * ∑(jacobian_element(left_op, ex_ketop(5, 6, 7, 8), H) * psym_tensor("ct", 5:8...), 5:8))
    @time begin
    ...
    γ = simplify_heavy(jacobian_element(left_op, b', H) * real_tensor("cγ"))
    @time begin
    ...
    s1 = simplify_heavy(∑(jacobian_element(left_op, ex_ketop(5, 6) * b', H) * real_tensor("cs", 5, 6), 5:6))
    @time begin
    ...
    s2 = simplify_heavy(1 // 2 * ∑(jacobian_element(left_op, ex_ketop(5, 6, 7, 8) * b', H) * psym_tensor("cs", 5:8...), 5:8))
    @time begin
